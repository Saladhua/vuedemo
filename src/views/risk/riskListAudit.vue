<template>
    <div>
        ...
        <el-table :data="tableData" border ref="tableRef" style="width: 100%" :height="tableHeight" :header-cell-style="headerStyle" :cell-style="headerCellStyle" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="auditState" label="审核状态" width="300" :show-overflow-tooltip="true">
              <template slot-scope="scope">
                <audit-status :data="scope.row" stateField="auditState" reasonField="rejectReason"></audit-status>
              </template>
            </el-table-column>
            <el-table-column label="操作" fixed="right" align="center" width="300" v-if="审核">
                <template slot-scope="scope">
                    <el-popover :ref="'agree'+scope.row.id" placement="top" width="180">
                        <p class='audit-pass-title'>确定审核通过吗?</p>
                        <div style="text-align: right; margin: 0" class="el-form">
                            <el-button type="primary" v-on:click="$refs['agree'+scope.row.id].doClose()">取消</el-button>
                            <el-button type="primary" v-on:click="$refs['agree'+scope.row.id].doClose();audit(scope.row);">确定</el-button>
                        </div>
                        <el-button slot="reference" type="text" size="small" class="el-icon-edit-outline">通过</el-button>
                    </el-popover>
                    <el-popover :ref="scope.row.id" placement="top" width="180">
                        <div>
                            <el-input type="textarea" :autosize="{ minRows: 10, maxRows: 12 }" v-model="rejectReason" placeholder="请填写不通过原因">
                            </el-input>
                        </div>
                        <div style="text-align: right; margin: 10px" class="el-form">
                            <el-button type="primary" v-on:click="$refs[scope.row.id].doClose();RefuseCancle();">取消</el-button>
                            <el-button type="primary" v-on:click="$refs[scope.row.id].doClose();unaudit(scope.row);">确定</el-button>
                        </div>
                        <el-button slot="reference" type="text" size="small" class="el-icon-edit-outline">不通过</el-button>
                    </el-popover>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    mounted() {
        ...
        //获取按钮权限
        let menus = this.publicFun.menusQX("/riskListAudit");
        this.config("风险专业", 1);
        this.config("风险等级", 2);
        this.config("事故类型", 3);
        console.log(menus);
    ...
    methods: {
        ...
        Department() { //选择部门
            ...
            openDialog({
                ...
                if (retCode) {
                    self.formInline.DutyDeptName = retCode[0].id
                    self.formInline.DutyDept = retCode[0].name
                } else {
                    self.formInline.DutyDeptName = ""
        ...
        //审核不通过
        unaudit(row) {
            unaudit({ id: row.id, reason: this.rejectReason }).then((res) => {
                if (res.code == 1) {
                    this.initPage();
                    this.$notify({
                        title: "成功",
