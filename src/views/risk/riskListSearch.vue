<template>
    <div>
        ...
        <template slot-scope="scope">
          <span v-if="scope.row.auditState == '审核通过'">{{scope.row.auditState}}</span>
          <el-tooltip v-if="scope.row.auditState == '审核不通过'" effect="dark" placement="top" :content="'不通过原因:' + scope.row.rejectReason">
            <span style="cursor: pointer">{{ scope.row.auditState }}</span>
          </el-tooltip>
          <span v-if=" scope.row.auditState != '审核通过' && scope.row.auditState != '审核不通过'">{{ scope.row.auditState }}</span>
        </template>
      </el-table-column> -->
...
export default {
    ...
    mounted() {
        ...
        //获取按钮权限
        let menus = this.publicFun.menusQX("/riskListSearch");
        this.config("风险专业", 1);
        this.config("风险等级", 2);
        this.config("事故类型", 3);
        console.log(menus);
